...
extension ViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        if let image = info[UIImagePickerControllerOriginalImage] as? UIImage {
            let originalImage = image.size.width
            let aspectRatio = originalImage / image.size.height
            var smallImage: CGSize
            if aspectRatio > 1 {
                smallImage = CGSize(width: 1000, height: 1000/aspectRatio)
            } else {
                smallImage = CGSize(width: 1000*aspectRatio, height: 1000)
            }
